...
def order_history_2(request):
    """Display the order history of the user."""
    orders = Order.objects.filter(user=request.user).prefetch_related('order_details').order_by('-order_date')
    return render(request, 'orders/order_history_2.html', {'orders': orders})

@login_required
def admin_manage_orders(request):
    """View for admins to see all orders from all users."""
    if not request.user.is_staff:
        return redirect('home_page')
    
    orders = Order.objects.all().prefetch_related('order_details', 'user').order_by('-order_date')
...
def select_address_for_order(request, order_id):
    ...
    return render(request, 'orders/select_address.html', {'order': order, 'addresses': addresses})
